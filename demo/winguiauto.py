...
def findTopWindows(wantedText=None, wantedClass=None, selectionFunction=None):
    ...
    for hwnd, windowText, windowClass in topWindows:
        if wantedText and not _normaliseText(wantedText) in _normaliseText(windowText).decode('gbk'):
            continue
...
def findControls(topHwnd,
    ...
    def searchChildWindows(currentHwnd):
        ...
        for childHwnd, windowText, windowClass in childWindows:
            ...
            if wantedText and \
               not _normaliseText(wantedText) in _normaliseText(windowText).decode('gbk'):
                continue
            if wantedClass and \
        ...
        if len(results)==0:
            print "窗体中的控件标题"
            for hwnd, windowText, windowClass in childWindows:
                print windowText
...
def getMenuInfo(hMenu, uIDItem):
    ...
    ctypes.windll.user32.GetMenuStringA(ctypes.c_int(hMenu),
                                        ...
                                        win32con.MF_BYPOSITION)
    # Added .decode('utf-8')
    menuInfo.name = menuName.value.decode('gbk')
...
#         activateMenuItem(notepadWindow, ['编辑', '替换'])
#     time.sleep(.5)
#     replaceDialog = findTopWindow(wantedText='替换')
#  
...
#     elif win_version in ["2K", "XP"]:
#         activateMenuItem(notepadWindow, ['格式', '字体'])
#     time.sleep(.5)
#     fontDialog = findTopWindow(wantedText='字体')
#  
...
#     time.sleep(.5)
#  
#     okButton = findControl(fontDialog, wantedClass="Button", wantedText="确定")
#     clickButton(okButton)
    ...
    print("Exit notepad")
    activateMenuItem(notepadWindow, ('文件', '退出'))
    time.sleep(.5)

    print("Don't save.")
#     saveDialog = findTopWindow(wantedClass='Notepad')
    saveDialog = findTopWindow(wantedText='记事本')
    time.sleep(.5)
    noButton = findControl(saveDialog, wantedClass="Button", wantedText="不保存(&N)")
